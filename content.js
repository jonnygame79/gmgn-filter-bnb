...
(function () {
    ...
    function init() {
        ...
        if (!window.location.href.includes('gmgn.ai/bsc/address/')) {
            return;
        }
    ...
    function startGetTokenInfo() {
        setInterval(async () => {
            try {
                ...
                tokenIndex++;
                const tokenDetail = await fetch(`https://gmgn.ai/api/v1/mutil_window_token_security_launchpad/bsc/${tokenAddress}?device_id=${device_id}&client_id=${client_id}&from_app=${from_app}&app_ver=${app_ver}&tz_name=${tz_name}&tz_offset=${tz_offset}&app_lang=${app_lang}&fp_did=${fp_did}&os=${os}`);
                ...
                const rug_info = await fetch(`https://gmgn.ai/api/v1/mutil_window_token_link_rug_vote/bsc/${tokenAddress}?device_id=${device_id}&client_id=${client_id}&from_app=${from_app}&app_ver=${app_ver}&tz_name=${tz_name}&tz_offset=${tz_offset}&app_lang=${app_lang}&fp_did=${fp_did}&os=${os}`, {
                    method: 'GET',
                ...
                const migration_info = await fetch(`https://gmgn.ai/api/v1/mutil_window_token_info?device_id=${device_id}&client_id=${client_id}&from_app=${from_app}&app_ver=${app_ver}&tz_name=${tz_name}&tz_offset=${tz_offset}&app_lang=${app_lang}&fp_did=${fp_did}&os=${os}`, {
                    method: 'POST',
                    body: JSON.stringify({ chain: "bsc", addresses: [tokenAddress] })
                });
...
https://gmgn.ai/api/v1/token_mcap_candles/bsc/${tokenAddress}?device_id=${device_id}&client_id=${client_id}&from_app=gmgn&app_ver=${app_ver}&tz_name=${tz_name}&tz_offset=${tz_offset}&app_lang=en-US&fp_did=${fp_did}&os=web&resolution=1m&from=${(first_tx_time - first_tx_time % 60) * 1000}&to=${first_tx_time * 1000 + 4 * 3600000}&limit=240`)
...
https://gmgn.ai/api/v1/token_mcap_candles/bsc/${tokenAddress}?device_id=${device_id}&client_id=${client_id}&from_app=gmgn&app_ver=${app_ver}&tz_name=${tz_name}&tz_offset=${tz_offset}&app_lang=en-US&fp_did=${fp_did}&os=web&resolution=1s&from=${first_tx_time * 1000 + 1000}&to=${first_sell_all_time * 1000}&limit=240`)
    ...
    function startTableRowMonitoring() {
        ...
        const observer = new MutationObserver(function (mutations) {
            if (!window.location.href.startsWith("https://gmgn.ai/bsc/address")) {
    ...
    function getCurrentFilters() {
        const savedFilters = localStorage.getItem('gmgn-bsc-filters');
        if (savedFilters) {
            try {
    ...
    function openMultipleBrowser() {
        ...
        if (filtered_tokens.length > 0) {
            while (opened_browsers < filtered_tokens.length) {
                ...
                window.open(`https://gmgn.ai/bsc/token/${filtered_tokens[opened_browsers]}?tag=All&maker=${wallet}&filter=All`, '_blank');
                opened_browsers++;
                cnt++;
                if (cnt == 7)
                    break;
            ...
            while (opened_browsers < tokens.length) {
                ...
                window.open(`https://gmgn.ai/bsc/token/${tokens[opened_browsers]}?tag=All&maker=${wallet}&filter=All`, '_blank');
                opened_browsers++;
                cnt++;
                if (cnt == 7)
                    break;
    ...
    function applyFilters() {
        ...
        localStorage.setItem('gmgn-bsc-filters', JSON.stringify(filtersData));
    ...
    function clearFilters() {
        ...
        // Clear localStorage
        localStorage.removeItem('gmgn-bsc-filters');
